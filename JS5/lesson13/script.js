...
/*Exercises - Start the Amazon Project, Intro to Git To do the exercises you can either:
1. Create a copy of the 'javascript-amazon-project' folder (for example, ‘13-javascript-amazon-project') and do the exercises in the copy. Advantage = exercises won't affect original folder.
2. Do the exercises directly in the 'javascript-amazon-project' folder. Advantage = you can combine everything together (everything we build in the course and everything we build in the exercises). Disadvantage = your code won't exactly match the video.
...
For the id, use any id that you want, like 'id', 'id2', etc. (just make sure its unique).
...
13h. We'll review some JavaScript shortcuts we can use. In amazon.js: Search for the code 'const productId = button.dataset.productld;' and use the destructuring shortcut to simplify it.
Search for 'cart.push({' and use shorthand property in the 2 lines below this code.


Challenge Exercises
We'll create the 'Added' message on the right.
...
131. After 2 seconds (use setTimeout), make the
message disappear by removing the class.
...
Modify the code so when we click 'Add to Cart', it "refreshes" the 2 second wait time (waits 2 seconds again and message disappears) Hint: you can cancel the previous setTimeout using clearTimeout()
*/